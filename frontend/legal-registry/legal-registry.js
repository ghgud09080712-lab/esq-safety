...
function switchView(view, options = {}) {
  ...
  if (refreshButton) refreshButton.hidden = view !== "dashboard";
...
function sortDetailCardsByRegistryOrder(cards) {
  ...
  (state.data.records || []).forEach((record, index) => {
    const order = { no: Number.parseInt(record.no, 10), index };
    [record.lawName, record.group].filter(Boolean).forEach((name) => {
      const key = normalizeSearchText(name);
      if (!recordOrder.has(key)) recordOrder.set(key, order);
    });
  });
